...
-- 명령 양식
CREATE TABLE 테이블이름(
	필드명1 DATATYPE [DEFAULT 값 OR 제약조건 등], -- [] 추가적으로 써주는것, 생략가능
...
CREATE TABLE booklist(
	booknum varchar2(5), -- variable char : 가변형
	subject varchar2(30) not null,
	makeyear number(4), -- (자릿수 = 길이)
...
-- 	CREATE TABLE
-- 	Create Table
-- 9. 보통은 데이터 형식과 용량(크기)를 지정하는데, DATE데이터형식은 유형을 별도로 크기를 지정하지 않음
...
-- 13. 컬럼(필드)과 컬럼(필드)의 구분은 콤마로 하되, 마지막 컬럼이나 constraint 를 작성후엔 콤마를 찍지 않음
...
-- 예를 들면 booklist에 존재 하지 않은 도서의 번호가 rentlist의 빌려간 도서의 번호로 등록되지못하게 하는것을 말합니다.


-- 테이블 생성 3
-- 테이블 이름 : rentlist
-- 필드 : rent_date(date), idx(NUMBER(3)), bnum(VARCHAR2(5)),
...
-- 외래키의 참조 대상이 되는 필드는 그쪽 테이블의 기본키이어야한다.
-- 예) membernum은 memberlist테이블의 primary key이어야 함.



-- 제약조건 (CONSTRAINT)
-- PRIMARY KEY
--	- 테이블에 저장된 레코드를 고유하게 식별하기 위한 키, 하나의 테이블에 하나의 기본키만 정의할 수
--	  있습니다.
...
--	- 단, NULL은 고유키 제약의 대상이 아니므로, NULL값을 가진 행 여러개가 UNIQUE KEY
--	  제약에 위반되지는 않습니다.
-- NOT NULL
...
--	  논리식을 지정합니다.
-- FOREIGN KEY
--	- 관계형 데이터 베이스에스 테이블간에 관계를 정의하기 위해 기본키를 다른 테이블의 외래키로
